#/bin/env pythonA
#-*- coding=utf-8 -*-
#random forest completement

import numpy as np
from collections import Counter, defaultdict
from functools import partial
import math, random, os, random

#RandomForest class
class RandomForest(object):
    def __init__(self, criterion='entropy', n_estimators=10, max_features='auto', max_depth=3, min_samples_split = 2,random_state=None):
    ...
    def fit(self, X, y, columns=None):
        ...
        n_samples = X.shape[0]
        selected_nsamples = math.ceil(n_samples/self.min_samples_split + 0.5) # force to return no less than 1
        #print ("%d samples are selected" %(selected_nsamples))
        all_features = X.shape[1]
        #max_feature to give feature number
        #auto
        if (self.max_features=='auto' or self.max_features=='sqrt'):
            self.n_features = math.ceil(math.sqrt(all_features))
        elif (self.max_features=='sqrt2'):
            self.n_features = math.ceil(math.log(all_features,2))
        else:
            self.n_features = all_features
        self.trees = []
        random.seed()
        for itree in range(self.n_estimators):
            #choose parts of the features from all features
            selected_columns = np.random.choice(self.columnname,self.n_features,replace=False)
            #boostrap for the training samples
            select_list=np.array([True,False])
            select_list=select_list.repeat([selected_nsamples, n_samples-selected_nsamples])
            np.random.shuffle(select_list)
            self.trees.append(self.build_tree(X=combinedX[select_list,:], split_candidates=selected_columns))
    ...
    def predict(self, X):
        y=[]
        for xi in X:
            votes=np.array([self.predict_single(xi,tree) for tree in self.trees])
            vote_counts = Counter(votes)
            y.append(vote_counts.most_common(1)[0][0])
            
        return np.array(y)

    #now the input tree shoulkd be multiple
    ...
    def build_tree(self, X, split_candidates=None, depth=0):
        ...
        #if no assigned column names, use all of the column names
